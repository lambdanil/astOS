...
def live_install(pkg):
    ...
    os.system(f"mount --rbind /dev /.overlays/overlay-{tmp}/dev >/dev/null 2>&1")
    os.system(f"mount --rbind /sys /.overlays/overlay-{tmp}/sys >/dev/null 2>&1")
    os.system(f"mount --bind /run /.overlays/overlay-{tmp}/run >/dev/null 2>&1")
    ...
    os.system(f"mount --rbind /proc /.overlays/overlay-{tmp}/proc >/dev/null 2>&1")
    os.system(f"mount --bind /etc/resolv.conf /.overlays/overlay-{tmp}/etc/resolv.conf >/dev/null 2>&1")
...
def live_unlock():
    ...
    #os.system(f"chattr -RV -i /.overlays/overlay-{tmp}/usr > /dev/null 2>&1")
    os.system(f"mount --bind /.overlays/overlay-{tmp} /.overlays/overlay-{tmp} > /dev/null 2>&1")
    os.system(f"mount --bind /run /.overlays/overlay-{tmp}/run >/dev/null 2>&1") # Pacman gets weird when chroot directory is not a mountpoint, so this unusual mount is necessary
    os.system(f"mount --bind /var /.overlays/overlay-{tmp}/var >/dev/null 2>&1")
    os.system(f"mount --rbind /dev /.overlays/overlay-{tmp}/dev >/dev/null 2>&1")
    os.system(f"mount --rbind /sys /.overlays/overlay-{tmp}/sys >/dev/null 2>&1")
    os.system(f"mount --rbind /tmp /.overlays/overlay-{tmp}/tmp >/dev/null 2>&1")
    os.system(f"mount --rbind /proc /.overlays/overlay-{tmp}/proc >/dev/null 2>&1")
    os.system(f"mount --bind /etc/resolv.conf /.overlays/overlay-{tmp}/etc/resolv.conf >/dev/null 2>&1")
